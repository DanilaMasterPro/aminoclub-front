...
const benefits = [
  ...
  ["delivery.svg", "Яндекс доставка", "Отправляем заказы по всей России"],
    ...
    <section
      ...
      {benefits.map(([icon, title, text], index) => (
        <article data-fade-up data-fade-up-delay={index * 0.06} className="flex items-center gap-5" key={title}>
          ...
          <div>
            <h2 className="mb-[7px] text-lg font-medium">{title}</h2>
            <p className="text-sm leading-[1.35] text-[#656a6c]">{text}</p>
          </div>
        </article>
      ))}
    </section>
